...
# Avoid circular build deps Test::YAML → Test::Base → YAML when bootstrapping
%if %{with perl_YAML_enables_test} && !%{defined perl_bootstrap}
%{?scl:scl enable %{scl} '}make test AUTHOR_TESTING=%{with perl_YAML_enables_extra_test}%{?scl:'}
%endif
...
- Avoid creation of the perllocal.pod file (%{?scl:scl enable %{scl} '}make pure_install).%{?scl:'}
